...
CLIV_E_NAME="${CLIV_E_NAME:-istioctl}"
CLIV_E_REV="0.1.0"
...
CLIV_E_GHREPOAPI="https://api.github.com/repos/istio/istio"
CLIV_E_BASEURL="https://github.com/istio/istio/releases/download/%s/${CLIV_E_NAME}-%s-%s-%s.tar.gz"
...
_ext_versions () {  cliv -E "$CLIV_E_NAME" -X versions_ghreleases "$CLIV_E_GHREPOAPI" | grep -v -e "-rc\|alpha\|beta" ;  }
_ext_unpack () {  cliv -E "$CLIV_E_NAME" -X unpack_untar "/usr/bin" ;  }
_ext_test () {  "$CLIV_E_INSTDIR/bin/$CLIV_E_BIN_NAME" version --remote=false --short 2>/dev/null 1>/dev/null ;  }
