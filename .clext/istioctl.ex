...
CLINST_E_NAME="${CLINST_E_NAME:-istioctl}"
CLINST_E_REV="0.1.0"
...
CLINST_E_GHREPOAPI="https://api.github.com/repos/istio/istio"
CLINST_E_BASEURL="https://github.com/istio/istio/releases/download/%s/${CLINST_E_NAME}-%s-%s-%s.tar.gz"
...
_ext_versions () {  clinst -E "$CLINST_E_NAME" -X versions_ghreleases "$CLINST_E_GHREPOAPI" | grep -v -e "-rc\|alpha\|beta" ;  }
_ext_unpack () {  clinst -E "$CLINST_E_NAME" -X unpack_untar "/usr/bin" ;  }
_ext_test () {  "$CLINST_E_INSTDIR/bin/$CLINST_E_BIN_NAME" version --remote=false --short 2>/dev/null 1>/dev/null ;  }
